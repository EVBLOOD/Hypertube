...
interface Peer { host: string; port: number; }
...
export class StreamsService implements OnModuleInit, OnModuleDestroy {
    ...
    async onModuleInit() {
        ...
        this.incomingServer = net.createServer((socket) => {
            ...
            wire.on('handshake', (infoHashBuffer: Buffer) => {
                ...
                this.setupWireListeners(wire, socket, infoHash, infoHashBuffer, key);
                wire.handshake(infoHashBuffer, this.peerId, { extended: true, dht: true });
            });
        });

        this.incomingServer.listen(LISTEN_PORT, '0.0.0.0', () => {
            console.log(`TCP Server is listening on port ${LISTEN_PORT} for incoming peers`);
        });
    }
    ...
    onModuleDestroy() {
        ...
        for (const [, pm] of this.downloads) {
            pm.destroy().catch(err => console.error('Could not destroy PieceManager due to error:', err));
        }

        if (this.chokeInterval) clearInterval(this.chokeInterval);
        if (this.optimisticUnchokeInterval) clearInterval(this.optimisticUnchokeInterval);
    ...
    private resolveVideoFile(metadata: any): VideoFile | undefined {
        ...
        if (!videoFiles.length) {
            const largest = files.reduce((a, b) => a.length > b.length ? a : b);
    ...
    private getSavedFilePath(imdbId: string, preferredQuality?: string): string | null {
        try {
            const files = require('fs').readdirSync(this.downloadDir);
            if (preferredQuality) {
                const exactPattern = new RegExp(`^${imdbId}-${preferredQuality}\\.`);
        ...
        try {
            ...
            if (BROWSER_SUPPORTED_FORMATS.includes(extension)) {
                ...
                await this.movieRepo.update(
                    { imdbId },
                    { filePath: destPath, isFullyDownloaded: true, lastWatchedAt: new Date() }
                ).catch(err => console.error('Failed to update movie DB:', err));
            } else if (extension === '.mkv'|| extension === '.mov') {
                const convertedPath = path.join(this.downloadDir, `${imdbId}-${quality}.mp4`);
                this.convertMKVtoMP4(sourcePath, convertedPath, imdbId, quality)
                    .catch(err => console.error(`[Conversion Error] ${imdbId}-${quality}:`, err)); 
            }
        } catch (err) {
    ...
    private async convertMKVtoMP4(
        ...
        return new Promise((resolve, reject) => {
            ...
            exec(ffmpegCmd, { timeout: 3600000 }, (err, _) => {
                if (err) {
                    console.error(`[Conversion Failed] ${imdbId}-${quality}: ${err.message}`);
                    reject(err);
                    return;
                }
                ...
                try {
                    ...
                    console.error(`[Cleanup Failed] Could not remove ${inputPath}:`, unlinkErr);
                    reject(unlinkErr);
                }
            });
        });
    ...
    async getMoviesWithSeedersQuality(imdbId: string, quality: string, res?: Response) {
        const torrents = await this.movieService.getTorrentMagnetsFromYTS(imdbId);
        if (!torrents?.length) {
            res?.status(404).json({ message: 'No torrent found for this title.' });
            return undefined;
        }
        ...
        const seeded = torrents
            .filter(t => t.seeds > 0)
            .sort((a, b) =>
                (b.quality === quality ? 1 : 0) - (a.quality === quality ? 1 : 0)
    ...
    async stream(imdbId: string, quality = '1080p', range?: string, res?: Response) {
        ...
        const hashWithQuality = existingHashs?.find(h => h.quality === quality);
        ...
        if (!hashWithQuality) {
            const chosen = await this.getMoviesWithSeedersQuality(imdbId, quality, res);
            if (!chosen) return;
            ...
            this.infoHashToImdbId.set(imdbId, [...(existingHashs ?? []), { infoHash, infoHashBuffer, announce, quality, parsedLength }]);
        } else {
        ...
        if (!this.connectedPeers.has(infoHash)) this.connectedPeers.set(infoHash, new Set());
        if (!this.pendingWires.has(infoHash)) this.pendingWires.set(infoHash, new Set());
        if (!this.activeWires.has(infoHash)) this.activeWires.set(infoHash, new Set());
        ...
        if (!this.activeTorrents.has(infoHash)) {
            ...
            try {
                this.startTracker(infoHash, infoHashBuffer, announce, parsedLength ?? 0);
        ...
        if (!videoFile) {
            res.status(500).json({ message: 'Could not resolve video file from torrent metadata.' });
            return;
        }

        void this.streamPiecesToResponse(infoHash, manager, videoFile, range, res, imdbId, quality)
            .catch(err => console.error('Error streaming pieces:', err));
    }

    private async streamLocalFile(filePath: string, range: string | undefined, res: Response): Promise<void> {
        try {
    ...
    private async streamPiecesToResponse(
        ...
        while (nextByteToSend <= absoluteEnd && !res.destroyed) {
            ...
            const pieceSize = pieceIndex === manager.totalPieces - 1
        ...
        if (manager.isComplete()) {
            ...
            console.log(`[Download Complete] ${imdbId}-${selectedQuality} - saving to disk...`);
            ...
            this.saveDownloadedFile(
                ...
                videoFile.extension,
                selectedQuality,
            ).catch(err => console.error('Error saving downloaded file:', err));
        }
    }
    ...
    private startTracker(
        ...
        tracker.on('update', (d: any) =>
            console.log(`Update from Tracker: ${d.complete} seeds / ${d.incomplete} leechers`));
        ...
        tracker.on('peer', (addr: string) => {
            ...
            console.log(`Found by Tracker peer ${host}:${port} for infoHash ${infoHash}`);
        ...
        if (this.trackers.has(infoHash)) {
            console.log(`[WARN] Replacing existing tracker for infoHash ${infoHash}`);
    ...
    private startDHT(infoHash: string, infoHashBuffer: Buffer) {
        this.dht.on('peer', (peer: any, ihBuf: Buffer) => {
            if (!Buffer.isBuffer(ihBuf) || ihBuf.toString('hex') !== infoHash) return;
            if (!peer?.host || isNaN(peer.port)) return;
            console.log(`[DHT] Found peer ${peer.host}:${peer.port} for infoHash ${infoHash}`);

            this.connectToPeer({ host: peer.host, port: peer.port }, infoHash, infoHashBuffer);
        });
    ...
    private connectToPeer(peer: Peer, infoHash: string, infoHashBuffer: Buffer) {
        const key = `${peer.host}:${peer.port}`;
        const peers = this.connectedPeers.get(infoHash)!;

        if (peers.has(key) || peers.size >= MAX_ACTIVE_PIECES || this.blacklistedPeers.has(key)) return;
    ...
    private setupWireListeners(wire: any, socket: net.Socket, infoHash: string, infoHashBuffer: Buffer, key: string) {
        const peers = this.connectedPeers.get(infoHash)!;
        ...
        wire.on('error', (e: Error) => { cleanupWire(); });
        wire.on('finish', () => { cleanupWire(); });
        ...
        wire.ut_metadata.on('metadata', async (metadataBuffer: Buffer) => {
            ...
            if (!fullMetadata?.pieces?.length || !fullMetadata.pieceLength) return;

            const videoFile = this.resolveVideoFile(fullMetadata);
            if (!videoFile) return;
            this.videoFiles.set(infoHash, videoFile);

            const imdbId = Array.from(this.infoHashToImdbId.entries()).find(([, hashList]) => hashList.some(h => h.infoHash === infoHash))?.[0];
            const storagePath = this.getStoragePath(imdbId || infoHash);

            const managerReady = await PieceManager.create(fullMetadata, storagePath);
        ...
        if (wire.ut_pex) {
            wire.ut_pex.on('peer', (addr: string) => {
                console.log(`PEX discovered peer ${addr} for infoHash ${infoHash}`);
                const sep = addr.lastIndexOf(':');
                const h = addr.slice(0, sep);
                const p = parseInt(addr.slice(sep + 1), 10);
                if (!h || isNaN(p)) return;
                this.connectToPeer({ host: h, port: p }, infoHash, infoHashBuffer);
            });
        }

        wire.on('handshake', (remoteInfoHash: Buffer) => {
            const remoteHex = Buffer.isBuffer(remoteInfoHash) ? remoteInfoHash.toString('hex') : String(remoteInfoHash);
        ...
        wire.on('bitfield', (bitfieldBuf: any) => {
            const actualBuffer = bitfieldBuf.buffer && !Buffer.isBuffer(bitfieldBuf) 
        ...
        wire.on('have', (index: number) => {
            ...
            if (!bf) {
                const len = pm ? Math.ceil(pm.totalPieces / 8) : Math.ceil((index + 8) / 8);
                bf = Buffer.alloc(len);
        ...
        wire.on('piece', async (index: number, offset: number, block: Buffer) => {
            this.inFlight.set(wire, Math.max(0, (this.inFlight.get(wire) ?? 1) - 1));

            const manager = this.downloads.get(infoHash);
            if (!manager) return;

            const originKey = `${infoHash}-${index}`;
            if (!this.blockOrigins.has(originKey)) this.blockOrigins.set(originKey, []);
            ...
            if (complete) {
                ...
                const queue = this.blockQueues.get(wire);
                if (queue) {
                    this.blockQueues.set(wire, queue.filter(b => b.index !== index));
                }
                ...
                if (wires) {
                    for (const w of wires) {
                        ...
                        if (queue) {
                            ...
                            try { w.cancel(index, 0, manager['getPieceSize'](index)); } catch { }
                        }
                    }
                }

                wires?.forEach(w => {
                    try { w.have(index); } catch { }
                });
    ...
    private fillPipeLine(wire: any, infoHash: string) {
            ...
            if (wires) {
                for (const otherWire of wires) {
                    ...
                    this.blockQueues.set(
                        otherWire,
                        otherQueue.filter(item => !released.includes(item.index))
                    );
                }
            }
        }
        ...
        while (activeSet.size < MAX_ACTIVE_PIECES) {
            const pieceIndex = manager.getNextSequentialPieceIndex(peerBitfield, bufferSizePieces, activeSet);
            ...
            const blocks = pieceStatus === 'downloading'
                ? manager.getPendingBlocksForPiece(pieceIndex)
                : manager.getBlocksForPiece(pieceIndex);
            for (const block of blocks) {
                queue.push({ index: pieceIndex, offset: block.offset, length: block.length });
            }
        }
        let fails = false;
        while ((this.inFlight.get(wire) ?? 0) < PIPELINE_DEPTH && queue.length > 0) {
            const block = queue.shift()!;
            wire.request(block.index, block.offset, block.length, (err: Error | null) => {
                if (err) console.debug(`[Pipeline] request failed piece=${block.index}`);
                fails = fails || !!err;
                ...
                if (err) {
                    ...
                    if (queue) {
                        this.blockQueues.set(
                            wire,
                            queue.filter(item => item.index !== block.index)
                        );
                    }
                }
            });
    ...
    private waitForManager(infoHash: string): Promise<PieceManager> {
        ...
        return new Promise((resolve, reject) => {
            ...
            const resolver = (pm: PieceManager) => { clearTimeout(timer); resolve(pm); };
            if (!this.metadataWaiters.has(infoHash)) this.metadataWaiters.set(infoHash, []);
