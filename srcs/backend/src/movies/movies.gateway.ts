...
export class MovieGateway {
    ...
    handleConnection(socket: Socket) {
        try {
            const authorization = socket.handshake.headers.authorization?.split(" ")[1];
            const payload = this.jwtService.verify(authorization || "");

            socket.join(`user:${payload.sub}`);
            this.connectedPlayers.set(payload.sub, [...(this.connectedPlayers.get(payload.sub) || []), socket]);
        } catch (err) {
            console.log(err)
            socket.disconnect();
            return;
    ...
    notifyHostInviteAccepted(hostId: string, roomId: string, guestId: string) {
        this.server.to(`user:${hostId}`).emit('INVITE_ACCEPTED', { roomId, guestId });
    ...
    handleJoinRoom(
        ...
        if (hostId == userId || guestId == userId) {
            ...
            const JoinedRoom = this.JoinedRooms.get(data.roomId) || new Map<string, Socket>();
            ...
            const JoinedRoomUpdated = this.JoinedRooms.get(data.roomId) || new Map<string, Socket>();
    ...
    handleSendMessage(
        @ConnectedSocket() client: Socket,
        @MessageBody() data: { roomId: string; content: string; movieId: string },
    ) {
        const [hostId, guestId] = this.roomsMembers.get(data.roomId) || [];
        const userId = client.handshake.headers.userId;
        ...
        if (hostId == userId || guestId == userId) {
            ...
            if (socket) {
                console.log(`User ${userId} sent message to room ${data.roomId}: ${data.content}`);
                socket.emit('MESSAGE', { content: data.content, sender: userId });
            }
    ...
    handleStartStream(
        ...
        if (hostId == userId || guestId == userId) {
            ...
            const socket = JoinedRoom?.get(userId != hostId ? hostId : guestId);
            if (socket) {
                console.log(`User ${userId} started stream in room ${data.roomId}`);
    ...
    handlePauseStream(
        ...
        if (hostId == userId || guestId == userId) {
            ...
            const socket = JoinedRoom?.get(userId != hostId ? hostId : guestId);
            if (socket) {
                console.log(`User ${userId} paused stream in room ${data.roomId}`);
