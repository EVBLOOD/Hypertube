...
export default function ProfilePage() {
    ...
    if (error) return <div>Profile unavailable.</div>

    

    const stats = data?.stats || { watched: 0, wishlisted: 0, liked: 0, disliked: 0, totalInteractions: 0 }
                ...
                <div className={styles.firstPartHolder}>
                    ...
                    <div className={styles.privateProfileSection}>
                        ...
                        <div className={styles.personalInfosField}>
                            <div className={styles.profilePicture}>
                                <img className={styles.avatarProfile} src={user?.avatar || "/hero.png"} alt="" width='128px' height='128px' />
                            </div>
                            <div className={styles.inputsholder}>
                                <InputCustom ref={userNameRef} placeHolder={'foo'} lableName='Director Alias'></InputCustom>
                                <InputCustom ref={userEmailRef} placeHolder={'email@example.com'} lableName='Secure Email'></InputCustom>
                            </div>

                        </div>
                        <ProfileSelectionInputs title='Public Preview' description='Hide primary email address from community members' />
                        <ProfileSelectionInputs title='System Language' description='Default interface and metadata localization' type='language' />

                        <ButtonCustom className={styles.submitChangesButton} textButton='COMMIT CHANGES' buttonImage={undefined} color='primary' onClick={handleSave}></ButtonCustom>
                    </div>
                    ...
                    <div className={styles.privateProfileSection}>
                        ...
                        <div className={styles.interactionsSection}>
                            {history.map((item: any) => <InteractionProfileCard key={item.id} />)}
                        </div>
                    </div>
                    <div>
                ...
                <div className={styles.secondPartHolder}>
                    ...
                    <div className={styles.logsContainer}>
                        ...
                        <div className={styles.SecurityLogs}>
                            <DescriptionComponent className={styles.discreptionRemoveMargin} text={`WATCHED: ${stats.watched} MOVIES`} />
                            <p  className={styles.discreptionRemoveMargin}>{stats.totalInteractions} INTERACTIONS</p>
                        </div>
                        <div className={styles.SecurityLogs}>
                            <DescriptionComponent className={styles.discreptionRemoveMargin} text={`LIKED: ${stats.liked} / DISLIKED: ${stats.disliked}`} />
                            <p  className={styles.discreptionRemoveMargin}>{stats.wishlisted} WATCHLATER</p>
                        </div>
                    </div>
                </div>
            </div>
